...
export default function SuratMasuk() {
    ...
    const [data, setData] = useState<KwitansiItem[]>(sampleData);
    const [sortConfig, setSortConfig] = useState<{
        key: keyof KwitansiItem;
        direction: 'asc' | 'desc';
    } | null>(null);
    const [showMenu, setShowMenu] = useState<number | null>(null);
    const [showModal, setShowModal] = useState(false);
    const [showViewModal, setShowViewModal] = useState(false);
    const [viewItem, setViewItem] = useState<KwitansiItem | null>(null);
    const [entriesPerPage, setEntriesPerPage] = useState<number>(10);
    const [currentPage, setCurrentPage] = useState<number>(1);
    const [formData, setFormData] = useState<Omit<KwitansiItem, 'id'> & { file?: File }>({
        nomorKwitansi: '',
        tanggal: '',
        penerima: '',
        nominalDiterima: '',
    ...
    const validateForm = () => {
        ...
        if (!formData.nomorKwitansi) newErrors.nomorKwitansi = 'Nomor kwitansi wajib diisi';
        if (!formData.tanggal) newErrors.tanggal = 'Tanggal wajib diisi';
        if (!formData.penerima) newErrors.penerima = 'Penerima wajib diisi';
        if (!formData.nominalDiterima) newErrors.nominalDiterima = 'Nominal diterima wajib diisi';
    ...
    const handleSort = (key: keyof KwitansiItem) => {
    ...
    const renderSortIcon = (key: keyof KwitansiItem) => {
    ...
    const handleDownload = (id: number) => {
        window.open(`/storage/kwitansi/${id}/download`, '_blank');
    };
    ...
    const handleExport = () => {
        const csvHeader = ['ID', 'Nomor Kwitansi', 'Tanggal', 'Penerima', 'Nominal Diterima', 'Tipe File', 'Nama File'];
        const csvRows = [
            csvHeader,
            ...data.map((item) => [item.id, item.nomorKwitansi, item.tanggal, item.penerima, item.nominalDiterima, item.tipeFile, item.namaFile]),
        ];
        ...
        link.href = url;
        link.setAttribute('download', 'data-kwitansi.csv');
    ...
    return (
        <AppLayout>
            <Head title="Data Kwitansi" />
            <div className="space-y-4 p-4">
                <h1 className="text-2xl font-bold">Data Kwitansi</h1>
                            ...
                            <table className="min-w-full divide-y">
                                <thead className="bg-gray-100">
                                    <tr>
                                        ...
                                        <th className="px-4 py-3 text-left text-sm font-semibold whitespace-nowrap">
                                            <button
                                                onClick={() => handleSort('nomorKwitansi')}
                                                className="hover:text-primary flex items-center transition-colors"
                                            >
                                                Nomor Kwitansi
                                                {renderSortIcon('nomorKwitansi')}
                                            </button>
                                        </th>
                                        <th className="px-4 py-3 text-left text-sm font-semibold whitespace-nowrap">
                                            <button
                                        ...
                                        <th className="px-4 py-3 text-left text-sm font-semibold whitespace-nowrap">
                                            ...
                                            >
                                                Penerima
                                                {renderSortIcon('penerima')}
                                            </button>
                                        </th>
                                        <th className="px-4 py-3 text-left text-sm font-semibold whitespace-nowrap">
                                            <button
                                                onClick={() => handleSort('nominalDiterima')}
                                                className="hover:text-primary flex items-center transition-colors"
                                            >
                                                Nominal Diterima
                                                {renderSortIcon('nominalDiterima')}
                                            </button>
                                        </th>
                                    ...
                                    {paginatedData.length > 0 ? (
                                        paginatedData.map((item) => (
                                            <tr key={item.id} className="hover:bg-gray-50">
                                                ...
                                                <td className="px-4 py-3 whitespace-nowrap">{item.nomorKwitansi}</td>
                                                <td className="px-4 py-3 whitespace-nowrap">{item.tanggal}</td>
                                                <td className="px-4 py-3 whitespace-nowrap">{item.penerima}</td>
                                                <td className="px-4 py-3 whitespace-nowrap">{item.nominalDiterima}</td>
                                                <td className="relative px-4 py-3 whitespace-nowrap">
                                                    ...
                                                    {showMenu === item.id && (
                                                        <div className="absolute right-4 z-10 mt-1 w-32 rounded border bg-white shadow-md">
                                                            <Link href={`/kwitansi/${item.id}/edit`}>
                ...
                {showModal && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
                        <div className="w-full max-w-md rounded bg-white p-6 shadow-lg">
                            <h2 className="mb-4 text-xl font-semibold">Tambah Data Kwitansi</h2>
                            <div className="space-y-3">
                                <div>
                                    <label className="mb-1 block text-sm font-medium">
                                        Nomor Kwitansi <RequiredStar />
                                    </label>
                                    <Input
                                        placeholder="Nomor kwitansi"
                                        value={formData.nomorKwitansi}
                                        onChange={(e) => setFormData({ ...formData, nomorKwitansi: e.target.value })}
                                    />
                                    {errors.nomorKwitansi && <p className="mt-1 text-xs text-red-500">{errors.nomorKwitansi}</p>}
                                </div>
                                <div>
                                    <label className="mb-1 block text-sm font-medium">
                                ...
                                <div>
                                    ...
                                    />
                                    {errors.penerima && <p className="mt-1 text-xs text-red-500">{errors.penerima}</p>}
                                </div>
                                <div>
                                    <label className="mb-1 block text-sm font-medium">
                                        Nominal Diterima <RequiredStar />
                                    </label>
                                    <Input
                                        placeholder="Nominal diterima"
                                        value={formData.nominalDiterima}
                                        onChange={(e) => setFormData({ ...formData, nominalDiterima: e.target.value })}
                                    />
                                    {errors.nominalDiterima && <p className="mt-1 text-xs text-red-500">{errors.nominalDiterima}</p>}
                                </div>
                                <div>
                                    <label className="mb-1 block text-sm font-medium">
                    ...
                    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
                        <div className="w-full max-w-2xl rounded bg-white p-6 shadow-lg">
                            <div className="mb-4 flex items-start justify-between">
                                <h2 className="text-xl font-semibold">Details Kwitansi</h2>
                            ...
                            <div className="mb-6 grid grid-cols-2 gap-4">
                                <div>
                                    <p className="text-sm text-gray-500">Nomor Kwitansi</p>
                                    <p className="font-medium">{viewItem.nomorKwitansi}</p>
                                </div>
                                <div>
                                    <p className="text-sm text-gray-500">Tanggal</p>
                                    <p className="font-medium">{viewItem.tanggal}</p>
                                </div>
                                <div>
                                    <p className="text-sm text-gray-500">Penerima</p>
                                    <p className="font-medium">{viewItem.penerima}</p>
                                </div>
                                <div>
                                    <p className="text-sm text-gray-500">Nominal Diterima</p>
                                    <p className="font-medium">{viewItem.nominalDiterima}</p>
                                </div>
                                <div>
