...
const sampleData: SuratMasukItem[] = [];
...
export default function SuratMasuk() {
    ...
    const [data, setData] = useState<SuratMasukItem[]>(sampleData);
    const [sortConfig, setSortConfig] = useState<{
        key: keyof SuratMasukItem;
        direction: 'asc' | 'desc';
    } | null>(null);
    const [showMenu, setShowMenu] = useState<number | null>(null);
    const [showModal, setShowModal] = useState(false);
    const [showViewModal, setShowViewModal] = useState(false);
    const [viewItem, setViewItem] = useState<SuratMasukItem | null>(null);
    const [entriesPerPage, setEntriesPerPage] = useState<number>(10);
    const [currentPage, setCurrentPage] = useState<number>(1);
    const [formData, setFormData] = useState<Omit<SuratMasukItem, 'id'> & { file?: File }>({
        pengirim: '',
        perihal: '',
        tanggal: '',
        tipeFile: '',
    ...
    const handleSort = (key: keyof SuratMasukItem) => {
    ...
    const renderSortIcon = (key: keyof SuratMasukItem) => {
    ...
    const handleDownload = (id: number) => {
        window.open(`/storage/surat-masuk/${id}/download`, '_blank');
    };
    ...
    const handleExport = () => {
        const csvHeader = ['ID', 'Pengirim', 'Perihal', 'Tanggal', 'Tipe File', 'Nama File'];
        const csvRows = [csvHeader, ...data.map((item) => [item.id, item.pengirim, item.perihal, item.tanggal, item.tipeFile, item.namaFile])];
        ...
        link.href = url;
        link.setAttribute('download', 'surat-masuk.csv');
                            ...
                            <table className="min-w-full divide-y">
                                <thead className="bg-gray-100">
                                    <tr>
                                        ...
                                        <th className="px-4 py-3 text-left text-sm font-semibold whitespace-nowrap">
                                            <button
                                                onClick={() => handleSort('tipeFile')}
                                                className="hover:text-primary flex items-center transition-colors"
                                            >
                                                Lampiran
                                                {renderSortIcon('tipeFile')}
                                            </button>
                                        </th>
                                    ...
                                    {paginatedData.length > 0 ? (
                                        paginatedData.map((item) => (
                                            <tr key={item.id} className="hover:bg-gray-50">
                                                ...
                                                <td className="px-4 py-3 whitespace-nowrap">{item.pengirim}</td>
                                                <td className="px-4 py-3 whitespace-nowrap">{item.perihal}</td>
                                                <td className="px-4 py-3 whitespace-nowrap">{item.tanggal}</td>
                                                <td className="px-4 py-3 whitespace-nowrap">
                                                    {item.namaFile ? (
                                                        <button onClick={() => handleView(item.id)} className="text-sm text-blue-600 hover:underline">
                                                            Lihat
                                                        </button>
                                                    ) : (
                ...
                {showModal && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
                        <div className="w-full max-w-md rounded bg-white p-6 shadow-lg">
                            <h2 className="mb-4 text-xl font-semibold">Tambah Surat Masuk</h2>
                            <div className="space-y-3">
                                <div>
                                    <label className="mb-1 block text-sm font-medium">
                                        Pengirim <RequiredStar />
                                    </label>
                                    <Input
                    ...
                    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
                        <div className="w-full max-w-2xl rounded bg-white p-6 shadow-lg">
                            <div className="mb-4 flex items-start justify-between">
                                <h2 className="text-xl font-semibold">Details Surat Masuk</h2>
